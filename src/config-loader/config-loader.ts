...
export interface SetupFunction {
	...
	(app: _express.Application, sbvrUtilsInstance: typeof sbvrUtils, db: Database, done: (err?: any) => void): void;
	(app: _express.Application, sbvrUtilsInstance: typeof sbvrUtils, db: Database, done?: (err?: any) => void): Promise<void> | void;
}
				...
				if (data.users != null) {
					...
					_.each(data.users, (user) => {
						...
						_.each(user.permissions, (permissionName) => {
							...
							permissionsCache[permissionName] = authApiTx.get({
								resource: 'permission',
								options: {
									$select: 'id',
									$filter: {
		...
		return Promise.map(configObj.models, (model) =>
			Promise.try<string | undefined>(() => {
				...
				if (model.migrationsPath) {
					...
					return readdirAsync(migrationsPath).map((filename) => {
