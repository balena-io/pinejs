...
export interface PinejsClient
	extends PinejsClientCoreFactory.PinejsClientCore<
		PinejsClient,
		Promise<{}>,
...
export function getAbstractSqlModel(
	request: Pick<ODataRequest, 'vocabulary'>,
): AbstractSqlModel;
export function resolveSynonym(
	request: Pick<ODataRequest, 'resourceName' | 'vocabulary'>,
): string;
export function resolveNavigationResource(
	request: Pick<ODataRequest, 'resourceName' | 'vocabulary'>,
	navigationName: string,
): string;
export function resolveOdataBind(odataBinds: OdataBinds, value: any): any;
export function getAffectedIds(args: {
	req: HookReq;
	request: HookRequest;
	tx: Tx;
}): Promise<number[]>;
export function addPureHook(
	method: string,
	vocabulary: string,
	resource: string,
	hooks: Hooks,
): void;
export function addSideEffectHook(
	method: string,
	vocabulary: string,
	resource: string,
	hooks: Hooks,
