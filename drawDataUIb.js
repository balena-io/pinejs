...
function serverURI(about, filters){
...
function uidraw(idx, objcb, pre, post, rootURI, pos, pid, filters, loc, even, ftree){
	...
	this.subRowIn = function(){
		...
		if(this.branch[0]=='col'){
			...
			this.targ = serverURI(this.about, this.filters);
			...
			this.pre += "<div class='panel' style='background-color:" + this.bg +	";'>"
			this.post += "</div>"
			targ = serverURI(this.about, this.filters);
			posl = targ;
			...
			switch(actn){
				...
				case "add":
					if(this.type == "term"){
						...
						res += "<input type='hidden' id='__serverURI' value='" + serverURI(this.about,[]) + "'>";
						...
						" onClick='addInst(" + "this.parentNode" + ",\"" + serverURI(this.about,[]) + 
								"\",\"" + targ + "\");return false;'>";
						res += "</form>";
						res += "</div>";
						...
						var addftcb = function(headers, result, parent){
							...
							if(trms.length == trmres.length){
								...
								res += "<div align='right'>";
								res += "<input type='submit' value='Submit This'" +
								" onClick='addInst(this.parentNode.parentNode,\"" + 
								serverURI(parent.about,[]) + "\",\"" + posl + 
								"\");return false;'>";
								res += "</div>";
								res += "</form>";
						...
						for(var j=0;j<this.schema.length;j++){
							if(this.schema[j][0]=='term'){
								var tar = serverURI(this.schema[j][1], this.filters);
				...
				case "edit":
					if(this.type == "term"){
						...
						serverRequest("GET", this.targ, '', [], function(headers, result, parent){
							...
							res += "<input type='hidden' id='__serverURI' value='" + 
										serverURI(parent.about,[]) + "." + id + "'>";
							res += "<input type='hidden' id='__backURI' value='" + serverURI(parent.about,[]) + "'>";
						...
						serverRequest("GET", targ, '', [], function(headers, result, parent){
							...
							var editftcb = function(headers, result, parent){
								...
								if(trms.length == trmres.length){
									...
									respr += "<input type='hidden' id='__serverURI' value='" + 
												serverURI(parent.about,[]) + "." + resu.instances[0].id + "'>";
									respr += "<input type='hidden' id='__backURI' value='" + 
												serverURI(parent.about,[]) + "'>";
									console.log('editfctp backURI='+serverURI(parent.about,[]));
									...
									"onClick='editInst(this.parentNode.parentNode, \"" + 
									serverURI(parent.about,[]) + "." + resu.instances[0].id +
									"\", \"" + serverURI(parent.about,[]) + "\");return false;'>";
									respo += "</div>";
									respo += "</form>";
									respo += "</div>"
							...
							for(var j=0;j<parent.schema.length;j++){
								if(parent.schema[j][0]=='term'){
									var tar = serverURI(parent.schema[j][1], parent.filters);
				...
				case "del":
					...
					res += "<input type='hidden' id='__serverURI' value='" + 
								serverURI(this.about,[]) + "." + this.id + "'>";
					res += "<input type='hidden' id='__id' value='" + this.id + "'>";
					res += "<input type='hidden' id='__type' value='" + this.about + "'>";
					res += "<input type='hidden' id='__backURI' value='" + serverURI(this.about,[]) + ">";
					console.log('del backURI='+serverURI(this.about,[])+' this.about='+this.about, this.ftree);
					res += "<input type='submit' value='Confirm' " +
					"onClick='delInst(this.parentNode.parentNode, \"" + 
					serverURI(this.about,[]) + "." + this.id +
					"\", \"" + serverURI(this.about,[]) + "\");return false;'>";
					res += "</form>";
...
function processForm(forma){
	...
	switch(action){
		...
		case 'addfctp':
			...
			break;	
		case 'del':
			denInst(forma,serverURI,backURI);
			break;
	}
}
