CodeMirror.defineMode("sbvr", function(config) {
	...
	return {
		copyState: function(state) {
			//We only copy state at the end of the line, this reduces the number of copy calls we need to make.
			if(state._codemirrorEOL === true) {
				state._codemirrorEOL = false;
		...
		token: function(stream, state) {
			var returnVal = function(stream, state) {
			...
			}.call(this, stream, state);
			if(stream.eol()) {
				state._codemirrorEOL = true;
			}
			return returnVal;
		},
