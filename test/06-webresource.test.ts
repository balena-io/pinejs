			...
			describe(`webresource ${resourcePath} - ${resourceName}`, () => {
				...
				it(`creates a organization with a large ${resourcePath}`, async () => {
					...
					const { largeStream, largeFileSize } = await getLargeFileStream(
						Math.min(
							intVar('PINEJS_WEBRESOURCE_MAXFILESIZE') * 0.8,
							MAX_SIZE_SBVR_CONSTRAINT * 0.95,
						) - fileSize,
						filePath,
					);
			...
			({
				...
				it('uploads a file via S3 presigned URL', async () => {
					...
					const { body: commitResponse } = (await api[model]
						...
						.expect(200)) as { body: WebResource };
...
const beginBlobUpload = async (
	...
	const { body } = (await api
		...
		.expect(200)) as { body: webResourceHandler.BeginMultipartUploadResponse };
