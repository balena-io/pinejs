...
describe('08 sbvrApi', function () {
	...
	describe('permissions.getUserPermissionsForRole', () => {
		...
		before(async () => {
			...
			await createRole('not-assigned-role', permissionIds);
		});
		...
		it(`should be an empty array if role does not exist`, async () => {
			const response = await supertest(testLocalServer)
				.get('/auth-test/getUserPermissionsForRole')
				.send({ userId: guestId, roleName: 'non-existing-role' });
			expect(response.body).to.deep.equal([]);
		});

		it(`should be an empty array if role exists but it is not assigned to user`, async () => {
			const response = await supertest(testLocalServer)
				.get('/auth-test/getUserPermissionsForRole')
				.send({ userId: guestId, roleName: 'not-assigned-role' });
			expect(response.body).to.deep.equal([]);
		});
	});
